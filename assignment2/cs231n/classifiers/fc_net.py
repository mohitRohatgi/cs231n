...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    out, sandwich_cache = affine_relu_forward(X, 
                self.params['W1'], self.params['b1'])
    scores, layer2_cache = affine_forward(out,
    ...
    l2_norm = np.sum(self.params['W1'] * self.params['W1'])
    l2_norm += np.sum(self.params['W2'] * self.params['W2'])
    loss += 0.5 * self.reg * l2_norm
    
    dout, grads['W2'], grads['b2'] = affine_backward(dout, layer2_cache)
    dout, grads['W1'], grads['b1'] = affine_relu_backward(dout, sandwich_cache)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    while layer < self.num_layers:
        ...
        if self.use_batchnorm:
            ...
            scores, cache = affine_batchnorm_relu_forward(
                            x, w, b, gamma, beta, self.bn_params[layer-1])
        else:
        ...
        layer += 1
        l2_norm += np.sum(w * w)
        l2_norm_grad += np.sum(w)
    
    w = self.params['W' + str(layer)]
    scores, cache = affine_forward(x, w, self.params['b' + str(layer)])
    cache_list.append(cache)
    l2_norm += np.sum(w * w)
    ...
    w = 'W' + str(layer)
    grads[w], grads['b' + str(layer)] = dw, db
    grads[w] += self.reg * self.params[w]
    layer -= 1
    
    while layer > 0:
        if self.use_dropout:
            dropout_cache, cache = cache_list.pop()
            cache_list.append(cache)
        ...
        if self.use_batchnorm:
          dout, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(
                                        dout, cache_list.pop())
          grads['gamma%d' % layer], grads['beta%d' % layer] = dgamma, dbeta
        else:
          dout, dw, db = affine_relu_backward(dout, cache_list.pop())
        w = 'W%d' % layer
        grads[w], grads['b%d' % layer] = dw, db
        grads[w] += self.reg * self.params[w]
        layer -= 1
    ############################################################################
    #                             END OF YOUR CODE                             #
